  ...
  <tbody class="virtual-floor">
    <tr :id="`virtual-floor-${virtualFloor.id}`">
      <td :colspan="(showVfJsonCol ? 1 : 0) + 9 + maxOutcomes">
        <Timeline :min="minVirtualFloorTimestamp" :start="Number(virtualFloor.creationTxTimestamp)"
          :open="virtualFloor.__typename === 'ClassicVirtualFloor' ? Number(virtualFloor.tOpen) : Number.NEGATIVE_INFINITY"
          :close="Number(virtualFloor.tClose)" :resolve="Number((virtualFloor.tResolve))"
          :max="maxVirtualFloorTimestamp" :locked="fastforwarding" :now="nextBlockTimestamp" />
        <div>
          <h2>
            {{ virtualFloor.title }}
          ...
          <p v-if="virtualFloor.__typename === 'ClassicVirtualFloor'">
          ...
          <div v-if="virtualFloor.__typename === 'ClassicVirtualFloor'">
    ...
    <tr>
      ...
      <td rowspan="2">
        <table>
          <tr :title="`creationTxTimestamp = ${virtualFloor.creationTxTimestamp}`">
            <th>creationTxTimestamp</th>
            <td>{{ formatTimestamp(creationTxTimestamp) }}</td>
          </tr>
          <tr v-if="virtualFloor.__typename === 'ClassicVirtualFloor'" :title="`tOpen = ${virtualFloor.tOpen}`">
            <th>tOpen</th>
...
<script lang="ts">
import {
  ClassicDoubleDiceApp as ClassicDoubleDiceAppContract,
...
import Outcome, { DoubleDiceApplicationContract } from './OutcomeComponent.vue'
import Timeline from './Timeline.vue'

@Options({
  props: {
    protocolContract: Object as PropType<DoubleDiceProtocolContract>,
    classicContract: Object as PropType<ClassicDoubleDiceAppContract>,
    randomContract: Object as PropType<RandomDoubleDiceAppContract>,
...
export default class VirtualFloorComponent extends Vue {
  ...
  randomContract!: RandomDoubleDiceAppContract
  classicContract!: ClassicDoubleDiceAppContract
  ...
  get contract(): DoubleDiceApplicationContract {
    if (this.virtualFloor.__typename === 'ClassicVirtualFloor') {
      return {
        commitToVirtualFloor: (vfId, outcomeIndex, amount) => this.classicContract.commitToVirtualFloor(vfId, [outcomeIndex], [amount], ethers.constants.MaxUint256),
        setResult: (vfId, outcomeIndex) => this.classicContract.finalizeUnsetResult(vfId, outcomeIndex)
      }
    } else {
      return {
        commitToVirtualFloor: (vfId, outcomeIndex, amount) => this.randomContract.commitToVirtualFloor(vfId, [outcomeIndex], [amount]),
      }
    }
  }

  get creationTxTimestamp(): number {
    return Number(this.virtualFloor.creationTxTimestamp)
  }

  get tOpen(): number {
    assert(this.virtualFloor.__typename === 'ClassicVirtualFloor')
  ...
  async cancelVirtualFloorUnresolvable(): Promise<void> {
    ...
    await tryCatch(async () => {
      const tx = await this.protocolContract.cancelVirtualFloorUnresolvable(this.virtualFloor.id)
      const { hash } = tx
  ...
  async claim(): Promise<void> {
    ...
    await tryCatch(async () => {
      ...
      if (preparedClaim.claimType === VirtualFloorClaimType.Payouts) {
        tx = await this.protocolContract.claimPayouts(this.virtualFloor.id, preparedClaim.tokenIds)
      } else {
        tx = await this.protocolContract.claimRefunds(this.virtualFloor.id, preparedClaim.tokenIds)
      }
      const { hash } = tx
      const txUrl = `https://polygonscan.com/tx/${hash}`
      console.log(`Sent ${txUrl}`)
