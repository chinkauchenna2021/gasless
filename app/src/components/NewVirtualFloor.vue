...
import { USER_ACCOUNT } from '@/config'
import {
...
import { generateRandomVirtualFloorId, toFixedPointEthersBigNumber, tryCatch } from '../utils'
...
@Options({
  props: {
    ...
    appContract: Object as PropType<ClassicDoubleDiceAppContract>,
...
export default class NewVirtualFloor extends Vue {
  ...
  appContract!: ClassicDoubleDiceAppContract
  ...
  get description(): string {
    ...
    const date = new Date(this.tResolve).toDateString() // ToDo: Fix conversion
  ...
  async createVirtualFloor(): Promise<void> {
    ...
    await tryCatch(async () => {
      ...
      const tx = await this.appContract.createVirtualFloor(params)
      const { hash } = tx
