...
async function main() {
  ...
  const ddContract = ClassicDoubleDiceApp__factory.connect(DOUBLEDICE_CONTRACT_ADDRESS, ddAdmin);
  for (const contract of contracts) {
    process.stdout.write(`Whitelisting ${await contract.name()}(${contract.address}) on DoubleDiceProtocol contract\n`);
