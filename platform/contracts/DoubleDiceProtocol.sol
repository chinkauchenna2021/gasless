    ...
    ExtraStorageGap
{
    ...
    /**
    * @notice outcomeIndexes == amounts not satisfied.
    */
     ...
     * @param outcomeIndexes An array of 0-based index of VF outcome to which to commit. Must be < nOutcomes.
     * @param amounts An array of amount of ERC-20 payment-token vf.paymentToken to commit.
     * @param optionalDeadline Latest timestamp at which transaction can be mined. Pass 0 to not enforce a deadline.
     */
    function commitToVirtualFloor(uint256 vfId, address committer, uint8[] memory outcomeIndexes, uint256[] memory amounts, uint256 optionalDeadline)
        external
        whenNotPaused
        nonReentrant
    {
        // Note: Each amounts and indexes should have the same length.
        if (amounts.length != outcomeIndexes.length) revert CommitmentMisMatch();
        ...
        uint256[] memory tokenIds = new uint256[](outcomeIndexes.length);
        UFixed256x18[] memory beta_e18 = new UFixed256x18[](outcomeIndexes.length);

        (uint256 minAmount, uint256 maxAmount) = vf.minMaxCommitmentAmounts();
        uint256 totalAmount = amounts.sumUint256Array();
        
        if (!((minAmount <= totalAmount) && (totalAmount <= maxAmount))) revert CommitmentAmountOutOfRange();

        for (uint i = 0; i < outcomeIndexes.length; i++) {
            ...
            UFixed32x6 beta = vf.application.betaOf(_vfId, outcomeIndex, timestamp);
            beta_e18[i] = beta.toUFixed256x18();
            ...
            tokenIds[i] = ERC1155TokenIds.vfOutcomeBetaIdOf(_vfId, outcomeIndex, beta);
        }
        ...
        // For this reason, the UserCommitment event is emitted before _mintBatch emits TransferSingle.
        emit UserCommitment({
            vfId: vfId,
            committer: committer,
