...
describe('ClassicDoubleDiceApp', function () {
  ...
  before(async function () {
    ...
    const helper = new DoubleDiceDeploymentHelper(ownerSigner);

    tokenUSDC = await helper.deployDummyUSDCoin();

    await helper.deployProxyAdmin();

    contract = await helper.deployUpgradeableDoubleDiceProtocol({
      constructorArgs: [],
      initializerArgs: [{
    ...
    appContract = await helper.deployUpgradeableClassicDoubleDiceApp({
      constructorArgs: [contract.address, tokenUSDC.address],
      initializerArgs: []
    });

    await helper.setup({ protocol: contract, classicApp: appContract });
...

    {
      ...
      const { logs } = await (await contract.connect(ownerSigner).updatePaymentTokenWhitelist(tokenUSDC.address, true)).wait();
      expect(logs).to.have.lengthOf(1);

      expect(findContractEventArgs(contract, logs, 'PaymentTokenWhitelistUpdate')).to.containSubset({
    ...
    await (await tokenUSDC.connect(user3Signer).approve(appContract.address, $(400))).wait();
    await (await tokenUSDC.connect(vfCreatorSigner).approve(contract.address, $(100))).wait();

    const vfId = 0x01ffffffffffffffff0000000000n; // lower 5 bytes must be all 00
    ...
    {
      await (await appContract.adjustCreationQuotas([{ creator: ownerSigner.address, relativeAmount: 1 }])).wait();
    }

    {
      await (await appContract.adjustCreationQuotas([{ creator: vfCreatorSigner.address, relativeAmount: 1000 }])).wait();
    }
...

    {
      ...
      const {
        logs,
        blockHash
      } = await (await appContract.createVirtualFloor({
        vfId,
      ...
      const vfCreatedEventArgs = findContractEventArgs(contract, logs, 'VirtualFloorCreation');
...

    {
      ...
      const { logs, blockHash } = await (await appContract.connect(user1Signer).commitToVirtualFloor(vfId, [outcomeIndex], [amount], UNSPECIFIED_COMMITMENT_DEADLINE)).wait();

      const vfCommitmentArgs = findUserCommitmentEventArgs(contract, logs);
      allUserCommitments.push(vfCommitmentArgs);

      const token1155TransferArgs = findContractEventArgs(contract, logs, 'TransferBatch');      
      ...
      // expect(token1155TransferArgs.values[0]).to.eq(amount);

      const nftIds = token1155TransferArgs.ids;
      // console.log(`nftId = ${nftId}`)

      expect(vfCommitmentArgs.vfId).to.eq(vfId);
      expect(vfCommitmentArgs.outcomeIndexes).to.eql([outcomeIndex]);
      expect(vfCommitmentArgs.amounts).to.eql([amount]);
      expect(vfCommitmentArgs.tokenIds).to.eql(nftIds);
    ...
    {
      const { logs } = await (await appContract.connect(user2Signer).commitToVirtualFloor(vfId, [1], [$(10)], UNSPECIFIED_COMMITMENT_DEADLINE)).wait();
      const userCommitmentArgs = findUserCommitmentEventArgs(contract, logs);
      allUserCommitments.push(userCommitmentArgs);
    }
    {
      const { logs } = await (await appContract.connect(user3Signer).commitToVirtualFloor(vfId, [1], [$(10)], UNSPECIFIED_COMMITMENT_DEADLINE)).wait();
      const userCommitmentArgs = findUserCommitmentEventArgs(contract, logs);
      allUserCommitments.push(userCommitmentArgs);
    }
    ...
    {
      const { logs } = await (await appContract.connect(user3Signer).commitToVirtualFloor(vfId, [2], [$(10)], UNSPECIFIED_COMMITMENT_DEADLINE)).wait();
      const userCommitmentArgs = findUserCommitmentEventArgs(contract, logs);
      allUserCommitments.push(userCommitmentArgs);
    }
    ...
    await (await contract.connect(user3Signer).safeTransferFrom(
      user3Signer.address,
      user2Signer.address,
      tokenIdOf({ vfId, outcomeIndex: 1, beta: betaAt('2032-01-01T02:00:01').div(10n ** 12n) }),
      $(5),
      '0x'
    )).wait();
    ...
    {
      const { logs } = await (await appContract.setResult(vfId, 1)).wait();
      const { vfId: eventVfId, operator, action, outcomeIndex } = findContractEventArgs(appContract, logs, 'ResultUpdate');
    ...
    {
      const { logs } = await (await appContract.confirmUnchallengedResult(vfId)).wait();
      {
        const { vfId: eventVfId, operator, action, outcomeIndex } = findContractEventArgs(appContract, logs, 'ResultUpdate');
        expect(eventVfId).to.eq(vfId);
        expect(operator).to.eq(ownerSigner.address);
      ...
      {
        const { state: resolutionState } = await appContract.resolutions(vfId);
...

      {
        const {
          ...
          protocolFeeAmount,
          creatorFeeAmount,
          winningOutcomeIndexFlags,
        } = findContractEventArgs(contract, logs, 'VirtualFloorResolutions');
        expect(winningOutcomeIndexFlags).to.eq(0b10);
        ...
        const allCommitmentBalanceIds = allUserCommitments.map(({ tokenIds }) => tokenIds[0]);
        ...
        {
          const { logs } = await (await contract.connect(user1Signer).claimPayouts(vfId, allCommitmentBalanceIds)).wait();
          contractBalanceAfterTx1 = await tokenUSDC.balanceOf(contract.address);
          const transferBatch = findContractEventArgs(contract, logs, 'TransferBatch');
          const [, , , ids, amounts] = transferBatch;
          console.log({ ids, amounts });
        }

        {
          const { logs } = await (await contract.connect(user2Signer).claimPayouts(vfId, allCommitmentBalanceIds)).wait();
        ...
        {
          const { logs } = await (await contract.connect(user3Signer).claimPayouts(vfId, allCommitmentBalanceIds)).wait();
        ...
        expect(contractBalanceAfterTx3).to.eq(3);
      }
    }
  });
  ...
  describe('Create Virtual Floor', function () {
    ...
    it('Should revert if time closure for vpf used in the past', async function () {
      ...
      await expect(
        appContract.createVirtualFloor({
          ...virtualFloorCreationParams,
          tOpen: pastTOpenTime,
    ...
    it('Should revert because VF creation quota is exceeded', async function () {
      ...
      await expect(appContract.connect(noQuotaSigner).createVirtualFloor({ ...params, creator: noQuotaSigner.address })).to.be.reverted;
    });

    it('Should revert if VF with same id created before', async function () {
      {
        await (await appContract.adjustCreationQuotas([{ creator: ownerSigner.address, relativeAmount: 2 }])).wait();
      }
      const params: VirtualFloorCreationParamsStruct = {
        ...virtualFloorCreationParams,
        paymentToken: tokenUSDC.address,
      ...
      await expect(appContract.createVirtualFloor(params)).to.be.revertedWith('DuplicateVirtualFloorId()');
    });
  });
  ...
  describe('Commit To Virtual Floor', function () {
    ...
    beforeEach(async () => {
      ...
      await (
        await appContract.connect(vfCreatorSigner).createVirtualFloor({ ...virtualFloorCreationParams, creator: vfCreatorSigner.address })
      ).wait();
    });
    ...
    it('Should generate unique token id for the granularity level of time slot duration after open time', async function () {
      ...
      const { logs: commitment1Events, blockHash: blockHash1 } = await (
        await appContract
          .connect(user1Signer)
          .commitToVirtualFloor(vfId1, [outcomeIndex], [amount], UNSPECIFIED_COMMITMENT_DEADLINE)
      ).wait();
      const commitment1EventArgs = findUserCommitmentEventArgs(contract, commitment1Events);
      ...
      const { logs: commitment2Events, blockHash: blockHash2 } = await (
        await appContract
          .connect(user1Signer)
          .commitToVirtualFloor(vfId1, [outcomeIndex], [amount], UNSPECIFIED_COMMITMENT_DEADLINE)
      ).wait();
      const commitment2EventArgs = findUserCommitmentEventArgs(contract, commitment2Events);
  ...
  describe('Resolve Virtual Floor', function () {
    ...
    before(async () => {
      ...
      const virtualFloorCreationParams: VirtualFloorCreationParamsStruct = {
        ...
        paymentToken: tokenUSDC.address,
        metadata: ENCODED_DUMMY_METADATA,
        creator: vfCreatorSigner.address
      };
      await (
        await appContract.connect(vfCreatorSigner).createVirtualFloor({ ...virtualFloorCreationParams, creator: vfCreatorSigner.address })
      ).wait();
      await (
        await appContract.connect(vfCreatorSigner).createVirtualFloor({
          ...virtualFloorCreationParams,
          vfId: vfId2,
          creator: vfCreatorSigner.address,
        })
      ...
      {
        await (await appContract.connect(user1Signer).commitToVirtualFloor(vfId, [1], [$(10)], UNSPECIFIED_COMMITMENT_DEADLINE)).wait();
        await (await appContract.connect(user3Signer).commitToVirtualFloor(vfId, [2], [$(10)], UNSPECIFIED_COMMITMENT_DEADLINE)).wait();
        await (await appContract.connect(user2Signer).commitToVirtualFloor(vfId2, [1], [$(10)], UNSPECIFIED_COMMITMENT_DEADLINE)).wait();
        await (await appContract.connect(user3Signer).commitToVirtualFloor(vfId2, [2], [$(10)], UNSPECIFIED_COMMITMENT_DEADLINE)).wait();
        await (await appContract.connect(user1Signer).commitToVirtualFloor(vfId3, [1], [$(10)], UNSPECIFIED_COMMITMENT_DEADLINE)).wait();
        await (await appContract.connect(user2Signer).commitToVirtualFloor(vfId3, [2], [$(10)], UNSPECIFIED_COMMITMENT_DEADLINE)).wait();
    ...
    it('Should revert if VF / market does not exist', async function () {
      ...
      await expect(appContract.connect(vfCreatorSigner).setResult(wrongVirtualFloorId, 1)).to.be.revertedWith(`WrongVirtualFloorState(${VirtualFloorState.None})`);
    });

    it('Should revert if resolve time has not reached', async function () {
      await expect(appContract.connect(vfCreatorSigner).setResult(vfId, 0)).to.be.revertedWith(`WrongVirtualFloorState(${VirtualFloorState.Active_Open_ResolvableLater})`);
    });

    it('Should set the VF result correctly with outcomes', async function () {
      await evm.setNextBlockTimestamp('2033-01-10T00:10:00');

      const { logs } = await (await appContract.connect(vfCreatorSigner).setResult(vfId, 1)).wait();
      const { vfId: eventVfId, operator, action, outcomeIndex } = findContractEventArgs(appContract, logs, 'ResultUpdate');
      expect(eventVfId).to.eq(vfId);
    ...
    it('Should revert if you try to confrim challenge before 1 hour is over', async function () {
      await expect(appContract.connect(user1Signer).confirmUnchallengedResult(vfId)).to.be.revertedWith('TooEarly()');
    });

    it('Should revert if you try to challenge a set result when it is too late', async function () {
      const checkpoint = await EvmCheckpoint.create();
      await (await appContract.connect(vfCreatorSigner).setResult(vfId3, 1)).wait();
      await evm.setNextBlockTimestamp('2033-01-12T01:10:00');

      await expect(appContract.connect(user1Signer).challengeSetResult(vfId3, 2)).to.be.revertedWith('TooLate()');
      await checkpoint.revertTo();
    });
    ...
    it('Should challenge a set result', async function () {
      ...
      await (await appContract.connect(vfCreatorSigner).setResult(vfId2, 1)).wait();
      ...
      const { logs } = await (await appContract.connect(user3Signer).challengeSetResult(vfId2, 2)).wait();
      const { vfId, operator, action, outcomeIndex } = findContractEventArgs(appContract, logs, 'ResultUpdate');
    ...
    it('Confirm unchallenged result by anyone after 1 hour', async function () {
      ...
      const { logs } = await (await appContract.connect(user1Signer).confirmUnchallengedResult(vfId)).wait();
    ...
    it('Should revert if vf creator tries to finalize result', async function () {
      const OPERATOR_ROLE = await appContract.OPERATOR_ROLE();
      await expect(appContract.connect(vfCreatorSigner).finalizeChallenge(vfId2, 2)).to.be.revertedWith(`AccessControl: account ${vfCreatorSigner.address.toLowerCase()} is missing role ${OPERATOR_ROLE}`);
    });
    ...
    it('Should finalize set result to favor challenger', async function () {
      ...
      const { logs } = await (await appContract.connect(ownerSigner).finalizeChallenge(vfId2, 2)).wait();
      const { vfId, operator, action, outcomeIndex } = findContractEventArgs(appContract, logs, 'ResultUpdate');
      const challengerBalanceAfterResolution = await tokenUSDC.balanceOf(user3Signer.address);
    ...
    it('Should finalize set result to favor vf creator', async function () {
      ...
      const challengerBalanceBeforeResolution = await tokenUSDC.balanceOf(user3Signer.address);
      const VFCreatorBalanceBeforeResolution = await tokenUSDC.balanceOf(vfCreatorSigner.address);
      const { logs } = await (await appContract.connect(ownerSigner).finalizeChallenge(vfId2, 1)).wait();
      const { vfId, operator, action, outcomeIndex } = findContractEventArgs(appContract, logs, 'ResultUpdate');
      const challengerBalanceAfterResolution = await tokenUSDC.balanceOf(user3Signer.address);
      const VFCreatorBalanceAfterResolution = await tokenUSDC.balanceOf(vfCreatorSigner.address);
      expect(vfId).to.eq(vfId2);
    ...
    it('Should finalize set result to favor no one', async function () {
      ...
      const challengerBalanceBeforeResolution = await tokenUSDC.balanceOf(user3Signer.address);
      const VFCreatorBalanceBeforeResolution = await tokenUSDC.balanceOf(vfCreatorSigner.address);
      const { logs } = await (await appContract.connect(ownerSigner).finalizeChallenge(vfId2, 0)).wait();
      const { vfId, operator, action, outcomeIndex } = findContractEventArgs(appContract, logs, 'ResultUpdate');
      const challengerBalanceAfterResolution = await tokenUSDC.balanceOf(user3Signer.address);
      const VFCreatorBalanceAfterResolution = await tokenUSDC.balanceOf(vfCreatorSigner.address);
      expect(vfId).to.eq(vfId2);
    ...
    it('Should let platform admin set result that was not set by vf owner', async function () {
      const checkpoint = await EvmCheckpoint.create();
      await evm.setNextBlockTimestamp('2033-01-12T01:10:00');
      const { logs } = await (await appContract.connect(ownerSigner).finalizeUnsetResult(vfId3, 2)).wait();
      const { vfId, operator, action, outcomeIndex } = findContractEventArgs(appContract, logs, 'ResultUpdate');
